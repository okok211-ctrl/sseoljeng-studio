...
## v23.48
- 일본어 SRT를 자막 블록별 독립 API 요청으로 번역하여 블록 병합/한 칸 밀림 방지
- 이전/다음 블록은 문맥 참고로만 제공하고 번역 대상 범위만 출력하도록 강제
- 번역 길이/형식/한글 잔존 검증 및 최대 3회 자동 재시도
- 최대 4개 동시 번역으로 속도와 안정성 균형


## v23.49
- 본문 이미지 수 선택을 10 / 15 / 20 / 25 / 30 / 35 / 40장으로 확장했습니다.
- 기본 이미지 수를 10장으로 변경했습니다.
- 이미지 수가 문단 수보다 많아도 최대 40장까지 생성 가능하도록 배분 로직을 수정했습니다.
- 같은 문단이 여러 이미지에 배정될 때 동일 장면 복제를 금지하고 서로 다른 순간·행동·감정·구도를 사용하도록 프롬프트를 보강했습니다.
- 기존 일본어 SRT, 원고, Vrew, 음성 검사, 자동 렌더 기능은 유지했습니다.

## v23.52 TTS 대화 화자 명확화 패치
- GPT 분할 생성 마법사 프롬프트에 TTS 화자 명확화 규칙 추가.
- 화자가 바뀌는 첫 대사 전후 1문장 안에 이름·관계 호칭·1인칭 주어를 명시하도록 강제.
- 두 명 이상 장면에서 주어 없는 연속 대사 금지.
- '그가/그녀가/상대가'보다 남편·시어머니·시누이·제가 같은 명확한 호칭 우선.
- 대사만 연속되어 화자를 추측해야 할 경우 행동/나레이션으로 화자 재확인.
- 자동 본문 생성 경로에도 동일 TTS 화자 규칙 적용.
